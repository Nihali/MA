...
#MLR
mlrmodel1 = lm(formula = sales ~ price + promotion, data=omni) #Multiple linear case
mlrmodel2 = lm(formula = sales ~ price, data=omni) #single linear case

summary(mlrmodel1) #All the data has starts so all 3 are significant 
summary(mlrmodel2)
#if the price increases by 1 unit, sales decreases by 53 units, keeping sales constant

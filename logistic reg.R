...
head(Default)
attach(Default) #the 4variable (defualt,student, balance, income) can directly accessed. No need to use $ sign to access a column
str(Default)
Default
dim(Default)
...
tmp = table(default) #Count of yes and no
tmp
333/9667
...
logit = glm(default ~ income + balance + student, family='binomial', data=Default) #default gives yes and no values and family also gives yes and no values like 0 and 1 in binomial form
summary(logit)
#StudentYes given as student column is categorial
#equation is Default: -1.087e+0.01 + 5.737e-0.03(balance) + (-6.4)Student(yes)
#Default: -1.087e+0.01 + 5.737e-0.03(balance) + (0)Student(yes)

#The values are in log scale, hence, we need to take the anti-log to get it in linear scale
exp(coef(logit))
#if student unit increases by 1 unit then the probability of default decreases by 1 unit
...
slice(Default, seq(1,n(),1000)) #sequence 1 to 1000 in the interval of 500 ie slicing it at every 500 row, dplyr command
head(ndata)
...
ndata
ndata %>% mutate(predict = ifelse(fitted.results < 0.5, 0,1)) #mutate - for adding another column of name predict based on some values, if fitted result < 0.5 then predicit it to 0 or else 1

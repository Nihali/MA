...
summary(USvote)
dim(USvote)
USvoteS = USvote[sample(1:nrow(USvote), 1000),] #from 1 to 1000 rows the sample is taken

#control parameters
ctrl <- chaid_control(minsplit = 200,minbucket = 5, minprob = 0) #only when we have 200 observations only then we can split, 5 nodes should be there, 
